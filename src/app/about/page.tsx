...
export default function AboutPage() {
  return (
    <div className="bg-white min-h-screen selection:bg-primary-gold/20 pt-20 overflow-hidden">
      {/* Cinematic Header */}
      <section className="relative py-32 md:py-48 flex items-center justify-center overflow-hidden bg-off-white">
        ...
        <div className="container-custom relative z-10 text-center">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="text-6xl md:text-9xl font-bold mb-10 leading-[0.85] tracking-tighter text-foreground font-heading animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-100">
              THE LEGACY OF <br />
              <span className="text-primary-gold italic font-heading">BEYOND SERVICE</span>
            </h1>
      ...
      <section className="section-padding bg-white">
        <div className="container-custom grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          ...
          <div className="space-y-8">
            ...
            <h2 className="text-4xl md:text-6xl font-bold leading-none tracking-tighter text-foreground font-heading">
              Crafting <br /> <span className="text-muted-foreground/30 font-light italic font-heading">A New Standard</span>
            </h2>
            ...
            <div className="pt-8">
               <Link href="/services" className="bg-foreground text-background px-12 py-5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-primary-gold hover:text-foreground transition-all shadow-premium">Explore Our Work</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-off-white border-y border-border-subtle">
        <div className="container-custom grid grid-cols-2 lg:grid-cols-4 gap-12 text-center">
           {[
             ...
             <div key={i} className="flex flex-col gap-2 group">
                <span className="text-4xl md:text-7xl font-bold tracking-tighter text-muted-foreground/10 group-hover:text-primary-gold/30 transition-colors duration-700 line-clamp-1">{stat.value}</span>
                <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">{stat.label}</span>
             </div>
           ))}
        </div>
      </section>
      ...
      <section className="section-padding bg-white">
        <div className="container-custom text-center mb-24">
          ...
          <h2 className="text-4xl md:text-7xl font-bold tracking-tighter text-foreground font-heading">Core Values</h2>
        </div>
        ...
        <div className="container-custom grid grid-cols-1 md:grid-cols-3 gap-12">
           {[
             ...
             <div key={i} className={`p-12 premium-card group ${value.yOffset ? 'md:translate-y-12' : ''}`}>
                ...
                <h3 className="text-2xl font-bold mb-6 tracking-tighter text-foreground font-heading">{value.title}</h3>
                <p className="text-muted-foreground font-medium leading-relaxed text-sm opacity-80">{value.desc}</p>
             </div>
           ))}
        </div>
      </section>
      ...
      {/* Quote Section */}
      <section className="py-48 bg-off-white border-t border-border-subtle">
        <div className="container-custom text-center max-w-4xl mx-auto">
           <div className="text-muted-foreground/10 text-9xl mb-12 flex justify-center">
             <i className="fas fa-quote-left"></i>
           </div>
           <p className="text-3xl md:text-5xl font-light italic leading-tight text-foreground mb-12 font-heading">
